# :local_db — Room entities, DAOs, local data sources

Room is the source of truth for every read. This module's public boundary is the eight
`*LocalDataSource` classes, and they return **domain models only**.

Entities, DAOs, `FinFlowDatabase` and the projection rows in `database/model/` are all
`internal`, and the data-source constructors are `internal` too so a public class cannot leak a
DAO. Check 11 of `scripts/check-context.sh` fails if any of them is made public.
