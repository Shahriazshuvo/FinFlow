# :ui — design system, MVI base, route keys

Everything Compose. Features depend on this and on `:core`, nothing else.

Packages: `designsystem/` (theme, tokens, components), `ui/` (`MviViewModel`, contracts,
`ObserveAsEvents`, `AppError.toUserMessage`), `navigation/` (`@Serializable` route keys, so one
feature can navigate to another without depending on it).
